...
from src.lib.jsontomermaid import (
    convert_json_to_mermaid,
    escape_edge_text,
    escape_node_text,
)
...
def test_escape_node_text():
    ...
    assert (
        escape_node_text('test (with "everything")')
        == r"test \(with &quot;everything&quot;\)"
    )
...
def test_escape_edge_text():
    ...
    # Test parentheses escaping
    assert (
        escape_edge_text("test (with parentheses)") == "test &#40;with parentheses&#41;"
    )
    ...
    # Test combined escaping
    assert (
        escape_edge_text('test (with "everything")')
...
def test_mermaid_conversion(tmp_path):
    ...
    assert 'node1["Test State \\(initial\\)"]' in mermaid_content
