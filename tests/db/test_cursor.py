...
def test_cursor_acquisition_and_closure(mock_env_vars, mock_psycopg_connect, mock_connection, caplog):
...
def test_cursor_attributes_accessible(mock_env_vars, mock_psycopg_connect, mock_connection):
...
def test_cursor_closure_after_exception(mock_env_vars, mock_psycopg_connect, mock_connection):
...
def test_cursor_creation_with_closed_connection(mock_env_vars, mock_psycopg_connect, mock_connection):
...
def test_cursor_operations_after_connection_loss(mock_env_vars, mock_psycopg_connect, mock_connection, caplog):
...
def test_cursor_error_propagation(mock_env_vars, mock_psycopg_connect, mock_connection, caplog):
    ...
    for error_class, message in error_types:
        ...
        with pytest.raises(error_class if isinstance(error_class, type(Error)) else ProgrammingError) as exc_info:
            with db.get_cursor() as cursor:
                pass
...
def test_cursor_cleanup_after_errors(mock_env_vars, mock_psycopg_connect, mock_connection):
